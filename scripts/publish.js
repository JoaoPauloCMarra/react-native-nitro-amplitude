...
const packageFilter = "react-native-nitro-amplitude";
...
function assertReleaseDocs(version) {
  ...
  if (!readme.includes(`react-native-nitro-modules`) || !readme.includes(nitroPeer)) {
    failures.push(
  ...
  if (
    !readme.includes("Experiment.initializeWithAmplitudeAnalytics") ||
    !readme.includes("AmplitudeWorker")
  ) {
    failures.push(
      "README.md is missing Amplitude analytics/experiment API examples",
    );
  }
...
`);
  ...
  console.log("");
  log("📦 Publishing react-native-nitro-amplitude", "bold");
  console.log("");
  ...
  if (isDryRun) {
    ...
    log(`   https://www.npmjs.com/package/react-native-nitro-amplitude`, "cyan");
  }
